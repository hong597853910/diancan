<template lang="html">
  <div class="">
    <transition name="transShow">
      <div class="cart " v-if="totalCount" @click="listToggle">
          <div class="badge" v-show="totalCount">
              {{totalCount}}
          </div>
          <i class="icon iconfont icon-gouwuche cartIcon"></i> 
      </div>
    </transition>
...
.cart
    ...
    background #dedede
    line-height 48px
    border #000 solid #d9dde1
    &:hover
      opacity 0.8 
    &.transShow-enter-active,&.transShow-leave-active
      opacity 0
    &.transShow-enter,&.transShow-leave-active
      transition all 0.5s
    .badge
          position absolute
          top: 0;
